...
/** Aviary-tier "choose your bird": honor a specific species while keeping the
 *  assignment counter (card badge numbers) monotonic. Does NOT touch the
 *  no-repeat queue — chosen birds sit outside the random rotation. */
export function chooseBird(id) {
  const bird = getBird(id);
  if (!bird) return null;
  state.totalAssigned = (state.totalAssigned || 0) + 1;
  save(state);
  return { ...bird, assignmentNumber: state.totalAssigned, cycle: state.cycle || 1, chosen: true };
}
