...
export async function sendHeadshots({ to, bird, images, cardUrl, birdUrl }) {
  ...
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;color:#0d0f14;max-width:560px">
      ...
      <p style="margin:0 0 12px">Clean, professional, and — as promised — not a bird in sight. Tap a link below to download each one.</p>
      <table style="border-collapse:collapse;margin:0 0 14px">${rows}</table>
      <p style="margin:0 0 6px">Except one. You were paired with <strong>${bird?.name || "a bird"}</strong>, and it lives here:</p>
      ${birdUrl ? `<p style="margin:0 0 6px"><a href="${birdUrl}" style="color:#10a37f;font-weight:bold;text-decoration:none">⬇ You + your ${bird?.name || "bird"} (the keepsake shot)</a></p>` : ""}
