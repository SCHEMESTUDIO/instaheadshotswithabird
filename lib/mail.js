...
export async function sendHeadshots({ to, bird, results, card }) {
  ...
  if (card && card.startsWith("data:image/png;base64,")) {
    attachments.push({ filename: "bird-id-card.png", content: card.split(",")[1] });
  }
  ...
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;color:#0d0f14;max-width:520px">
      ...
      ${card ? `<p style="margin:0 0 12px">Your shareable <strong>Bird ID card</strong> is attached too.</p>` : ""}
