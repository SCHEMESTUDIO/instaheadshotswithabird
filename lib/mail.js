...
export async function sendHeadshots({ to, bird, images, cardUrl, cleanUrl }) {
  ...
  await sendViaResend({ to, subject: "Your Headshots with a Bird", html });
}

// The basic tier's ONE bird-free pick, sent at pick time (the pick is the
// final step of the results flow — delivered by email, not a spinner).
export async function sendCleanPick({ to, bird, url }) {
  const html = `
    <div style="font-family:Arial,Helvetica,sans-serif;color:#121212;max-width:560px">
      <h2 style="margin:0 0 8px">Your bird-free pick is ready.</h2>
      <p style="margin:0 0 12px">You chose one headshot to lose its <strong>${bird?.name || "bird"}</strong>. It has been removed. This was final — the other shots keep theirs.</p>
      <p style="margin:0 0 14px"><a href="${url}" style="color:#0D7A5C;font-weight:bold;text-decoration:none">Download your bird-free headshot</a></p>
      <p style="margin:0 0 12px;color:#8C877B;font-size:13px">Save it soon — download links don't stay live forever.</p>
      <p style="margin:16px 0 0;color:#8C877B;font-size:12px">headshotswithabird.com</p>
    </div>`;
  await sendViaResend({ to, subject: "Your bird-free pick — Headshots with a Bird", html });
}
