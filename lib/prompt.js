...
export function buildTwoPersonPrompt(look, bird) {
  return [
    ...
    "RULE 1 — FULL RE-RENDER (mandatory): produce a brand-new photorealistic studio headshot by generating every pixel from scratch. Do NOT cut out, crop, paste, warp, or collage any part of the reference photos. The result must look like the person sat for a fresh studio photo shoot on a different day — a new frame, not an edit of the originals.",
    "RULE 2 — EXACT LIKENESS (mandatory): the newly rendered person must match the references precisely in every feature — bone structure, face shape, eyes and eye colour, nose, mouth, skin tone, apparent age, and beard/facial hair.",
    "RULE 3 — HAIR IS CRITICAL: reproduce their exact hairline, hair length, density and amount, including any baldness, receding, or thinning, EXACTLY as shown. Never add, restore, thicken, lengthen, or restyle hair. If they are bald or balding, they stay exactly that bald.",
    "RULE 4 — NO BEAUTIFICATION: render natural, realistic skin with real texture. Do NOT airbrush, smooth, slim, de-age, or beautify. Accuracy over flattery.",
    "Pose: a natural, relaxed head-and-shoulders pose with a genuine natural smile, under flattering professional studio lighting.",
    `Style: dress them in ${look.outfit}; set the background to ${look.background}.`,
...
export function buildPrompt(look, bird) {
  return [
    ...
    `Change ONLY these three things: (1) dress them in ${look.outfit}, fitted naturally; (2) replace the background with ${look.background}; (3) add exactly one bird — ${bird.look} — perched naturally on the person's shoulder, in sharp photorealistic focus so it looks like it belongs in the photo.`,
